...
fn parse_date<'a>(
    now: &DateTime<Tz>,
    source: &'a str,
) -> Result<Parse<'a, DateTime<Tz>>, ParseError> {
...
fn parse_month_day_date<'a>(
    ...
    let date = now
        ...
        .unwrap()
        .with_month(month_parse.result)
        .unwrap()
        .with_day(date_parse.result)
        .unwrap();

    Ok(Parse {
        rest: date_parse.rest,
        result: date,
    })
}
